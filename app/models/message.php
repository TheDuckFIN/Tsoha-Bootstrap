<?php
	...
	class Message extends BaseModel {
		...
		public static function last_message_by_board_id($board_id) {
			$query = DB::connection()->prepare('SELECT m.id, m.sender_id, m.time, m.message, t.id AS thread_id FROM Message m INNER JOIN Thread t ON t.id = m.thread_id INNER JOIN Board b ON b.id = t.board_id WHERE b.id = :id ORDER BY m.id DESC LIMIT 1;');
